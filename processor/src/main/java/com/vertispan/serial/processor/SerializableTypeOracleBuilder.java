...
import com.vertispan.serial.processor.ProblemReport.Priority;
import com.vertispan.serial.processor.TypeParameterExposureComputer.TypeParameterFlowInfo;
import com.vertispan.serial.processor.TypePaths.TypePath;
...
//        JTypeParameter isTypeParameter = type.isTypeParameter();
    ...
    static boolean shouldConsiderFieldsForSerialization(SerializingTypes types, TypeMirror type, TypeFilter filter,
        ...
        if (isManuallySerializable(types, type)) {
            TypeElement manualSerializer = findCustomFieldSerializer(null, type);
            assert (manualSerializer != null);

            List<String> fieldProblems = CustomFieldSerializerValidator.validate(types.getTypes(), manualSerializer, (TypeElement) type);
